...
def main(args):
    ...
    if args.fine_tune and args.cont_training:
        ...
        for param, p_tmp in zip(model.dec_tower.parameters(), model_tmp.dec_tower.parameters()):
            param.data = p_tmp.data
            param.requires_grad = False
        for param, p_tmp in zip(model.dec_kv.parameters(), model_tmp.dec_kv.parameters()):
            param.data = p_tmp.data
            param.requires_grad = False
        for param, p_tmp in zip(model.dec_sampler.parameters(), model_tmp.dec_sampler.parameters()):
            param.data = p_tmp.data
            param.requires_grad = False
        model = model.cuda()
        if args.distributed:
            dist.barrier()
    elif args.cont_training:
    ...
    for epoch in range(init_epoch, args.epochs):
        if args.param_check:
            dec_tower_before = copy.deepcopy([p.data for p in model.dec_tower.parameters()])
            enc_tower_before = copy.deepcopy([p.data for p in model.enc_tower.parameters()])
            dec_sampler_before = copy.deepcopy([p.data for p in model.dec_sampler.parameters()])
            dec_kv_before = [p.data for p in model.dec_kv.parameters()]
        # update lrs.
        if args.distributed:
        ...
        if args.param_check:
            dec_tower_after = [p.data for p in model.dec_tower.parameters()]
            enc_tower_after = [p.data for p in model.enc_tower.parameters()]
            dec_sampler_after = [p.data for p in model.dec_sampler.parameters()]
            dec_kv_after = [p.data for p in model.dec_kv.parameters()]
            diff_tower = [torch.sum(b - a) for b, a in zip(dec_tower_before, dec_tower_after)]
            diff_enc_tower = [torch.sum(b - a) for b, a in zip(enc_tower_before, enc_tower_after)]
            diff_sampler = [torch.sum(b - a) for b, a in zip(dec_sampler_before, dec_sampler_after)]
            diff_kv = [torch.sum(torch.abs(b - a)) for b, a in zip(dec_kv_before, dec_kv_after)]
            print(f'Param diff dec tower (epoch {epoch}): {torch.stack(diff_tower, dim=0).sum(dim=0)}', flush=True)
            print(f'Param diff ENC tower (epoch {epoch}): {torch.stack(diff_enc_tower, dim=0).sum(dim=0)}', flush=True)
            print(f'Param diff dec sampler (epoch {epoch}): {torch.stack(diff_sampler, dim=0).sum(dim=0)}', flush=True)
            if len(diff_kv) > 0:
                print(f'Param diff dec kv (epoch {epoch}): {torch.stack(diff_kv, dim=0).sum(dim=0)}', flush=True)

            if diff_tower > torch.Tensor(0):
                logging.info(f'Param diff dec kv (epoch {epoch}): {torch.stack(diff_tower, dim=0).sum(dim=0)}')

            if diff_sampler > torch.Tensor(0):
                logging.info(f'Param diff dec kv (epoch {epoch}): {torch.stack(diff_sampler, dim=0).sum(dim=0)}')

            if diff_enc_tower == torch.Tensor(0):
                logging.info(f'Param diff dec kv (epoch {epoch}): {torch.stack(diff_sampler, dim=0).sum(dim=0)}')
...
def test(valid_queue, model, num_samples, args, logging):
    ...
    eval_time_elapsed = time() - start_time
    utils.average_tensor(nelbo_avg.avg, args.distributed)
    utils.average_tensor(neg_log_p_avg.avg, args.distributed)
    if args.distributed:
        # block to sync
        dist.barrier()
    logging.info('val, step: %d, NELBO: %f, neg Log p %f, time elapsed %f', step, nelbo_avg.avg, neg_log_p_avg.avg, eval_time_elapsed)
...
def init_processes(rank, size, fn, args, master_port):
    """ Initialize the distributed environment. """
    os.environ['MASTER_ADDR'] = args.master_address
    os.environ['MASTER_PORT'] = master_port
    torch.cuda.set_device(args.local_rank)
    print(f"Working dir: {os.getcwd()}")
    if 'oskarkviman' in os.getcwd():
        print("Server init process group")
        print(f"rank: {rank} - world size: {size} - master port: {os.environ['MASTER_PORT']}")
        dist.init_process_group(backend='nccl', init_method='env://', rank=rank, world_size=size)
        print(f"After init on rank {rank}")
    else:   # local
...
if __name__ == '__main__':
    ...
    if size > 1:
        ...
        for rank in range(size):
            ...
            p = Process(target=init_processes, args=(global_rank, global_size, main, args, master_port))
            p.start()
            processes.append(p)
