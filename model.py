...
### MISELBO Added Code start
    def forward_miselbo(self, x, z_in=None, s_in=None):
        if z_in is None:
            z_out = []
        if s_in is None:
            s_out = {}
        ...
        batch_size = z.size(0)
        s = s.expand(batch_size, -1, -1, -1)
        for c, cell in enumerate(self.dec_tower, 1):
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    if s_in is not None:
                        s = s_in[idx_dec]
        ...
        if z_in is not None:
            return logits, log_q, log_p, kl_all, kl_diag, None, None
        return logits, log_q, log_p, kl_all, kl_diag, z_out, s_out
### MISELBO Added Code end
